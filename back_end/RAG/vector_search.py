...
class Finder():
    ...
    def find(self,query,user_id):
        answer_dict = {}
        from entries_back_end.models import Entries
        query_vector = self.client.embed(query)
        queryset = Entries.objects.filter(owner=user_id).values("entry_vectors","entry_text","pk","title_text")

        for entry in queryset:
            similarity = self.cosine_similarity(entry["entry_vectors"],query_vector)
            answer_dict[similarity] = entry
            
        highest_similarity = heapq.nlargest(self.no_of_answers,answer_dict.keys())
        return [answer_dict[k] for k in highest_similarity if k>self.similarity_cutoff]
